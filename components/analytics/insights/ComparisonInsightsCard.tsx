...
type ComparisonInsightsCardProps = {
  title: string;
  lastLabel: string;
  rows: ComparisonInsightRow[];
};

const PAGE_SIZE_OPTIONS = [10, 12, 25, 50];

export default function ComparisonInsightsCard({
  title,
  lastLabel,
  rows,
        ...
        <div className="analytics-toolbar-row">
          <div className="analytics-toolbar-copy">
            ...
            <p className="analytics-card-description">
              Compare student accuracy against the class baseline in one readable grid.
            </p>
          </div>
        ...
        <table className="min-w-full text-sm">
          <thead className="bg-muted/30">
            <tr>
              ...
              <th className="analytics-th-center">Student Correct (%)</th>
              <th className="analytics-th-center">Class Correct (%)</th>
            ...
            {pagination.visibleRows.map((row) => {
              ...
              return (
                <tr key={row.tag} className="analytics-row">
                  ...
                  <td className="analytics-td-center analytics-table-measure-cell">
                    {row.studentCorrect == null
                      ? "-"
                      : row.studentCorrect.toFixed(2)}
                  </td>
                  <td className="analytics-td-center analytics-table-measure-cell">
                    {row.classCorrect == null ? "-" : row.classCorrect.toFixed(2)}
                  </td>
                  <td className={`analytics-td-center analytics-table-measure-cell font-medium ${gapClass}`}>
