...
const OverallPerformancePieChart = ({
  stats,
  title,
}: {
  stats: any;
  ...
  const total =
    getStatsSum(stats, "correct") +
    getStatsSum(stats, "incorrect") +
    getStatsSum(stats, "unattempted");
  if (total === 0) return null;

  const data = [
    {
      name: "Correct",
      value: getStatsSum(stats, "correct"),
      students: getStatsStudents(stats, "correctStudents"),
    },
    {
      name: "Incorrect",
      value: getStatsSum(stats, "incorrect"),
      students: getStatsStudents(stats, "incorrectStudents"),
    },
    {
      name: "Unattempted",
      value: getStatsSum(stats, "unattempted"),
      students: getStatsStudents(stats, "unattemptedStudents"),
    },
  ].filter((d) => d.value > 0);
...
const GroupedPerformanceBarChart = ({
  stats,
  groupBy,
}: {
  ...
  const chartData = Object.entries(stats)
    .filter(
      ([, value]) =>
        typeof value === "object" && value !== null && "correct" in value,
    )
    .map(([key, value]) => ({
      name: key,
...
const ChartView = ({
      ...
      ([, value]) =>
        ...
        ("correct" in value ||
          Object.values(value).some(
            (v: any) => v && typeof v === "object" && "correct" in v,
          )),
    );
  }
  ...
  async function handleDownloadChartImage() {
    ...
    try {
      ...
      const base =
        mode === "student" && studentName ? `student-${studentName}` : "class";
  ...
  // Download only the bar chart image
  async function handleDownloadBarChartImage() {
    ...
    try {
      ...
      const base =
        mode === "student" && studentName ? `student-${studentName}` : "class";
      const paper = paperTitle ? `-${paperTitle}` : "";
      a.download = `${base}${paper}-bar-chart-${ts}.png`;
      a.href = dataUrl;
      a.click();
    } catch (e) {
      console.error("Failed to download bar chart image", e);
    } finally {
      setDownloading(false);
    }
  }

  // Download only the pie charts grid image
  async function handleDownloadPieChartsImage() {
    ...
    try {
      ...
      const base =
        mode === "student" && studentName ? `student-${studentName}` : "class";
      const paper = paperTitle ? `-${paperTitle}` : "";
      a.download = `${base}${paper}-pie-charts-${ts}.png`;
      a.href = dataUrl;
      a.click();
    } catch (e) {
      console.error("Failed to download pie charts image", e);
    } finally {
      setDownloading(false);
    }
  }
  ...
  async function handleDownloadRemedials() {
    try {
      setDownloading(true);
      const metrics = buildStudentAreaMetrics(stats, groupBy, groupFields, {
        key: "",
        direction: "desc",
      });
      if (mode === "student") {
