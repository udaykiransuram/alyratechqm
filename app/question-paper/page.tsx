...
export default function QuestionPapersListPage() {
  ...
  useEffect(() => {
    const schoolKey =
      document.cookie.match(/(?:^|; )schoolKey=([^;]+)/)?.[1] || "";
  ...
  const classOptions = useMemo(
    () => [
      "All",
      ...Array.from(new Set(papers.map((p) => p.class?.name).filter(Boolean))),
    ],
    [papers],
  );
  ...
  const filteredPapers = useMemo(() => {
    let list =
      classFilter === "All"
        ? papers
        : papers.filter((p) => p.class?.name === classFilter);
    if (search.trim()) {
  ...
  const allFilteredChecked =
    ...
    filteredPapers.every((p) => selectedPaperIds.includes(p._id));
                  ...
                  <TableRow key={paper._id}>
                    ...
                    <TableCell>
                      <div className="flex flex-wrap gap-2">
                        <Link href={`/question-paper/view/${paper._id}`}>
                          <Button variant="outline" size="sm">
                            View
                          </Button>
                        </Link>
                        <Link href={`/question-paper/${paper._id}/responses`}>
                          <Button variant="outline" size="sm">
                            Responses
                          </Button>
                        </Link>
                        ...
                        <Link
                          href={`/analytics/class-tag-report/${paper._id}`}
                          prefetch={false}
                        >
