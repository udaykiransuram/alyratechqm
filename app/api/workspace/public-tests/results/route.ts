...
import { listWorkspacePublicTestResults } from "@/lib/server/workspace-public-tests";
...
export async function GET(req: NextRequest) {
  ...
  try {
    const results = await listWorkspacePublicTestResults({
    ...
    return NextResponse.json(
      {
        ...
        message:
          error instanceof Error
            ? error.message
            : "We couldn't load summer public-test results.",
      },
      { status: 400 },
    );
  }
