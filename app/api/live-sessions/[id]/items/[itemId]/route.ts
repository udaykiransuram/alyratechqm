...
import {
  deleteWorkspaceLiveSessionItem,
  getLiveSessionErrorStatus,
  normalizeLiveSessionItemWriteInput,
  updateWorkspaceLiveSessionItem,
...
export async function PATCH(req: NextRequest, { params }: RouteContext) {
  ...
  try {
    ...
    const liveSession = await updateWorkspaceLiveSessionItem({
    ...
    return NextResponse.json(
      {
        ...
        message:
          error instanceof Error
            ? error.message
            : "Failed to update the live item.",
      },
...
export async function DELETE(req: NextRequest, { params }: RouteContext) {
  ...
  try {
    const { id, itemId } = await params;
    const liveSession = await deleteWorkspaceLiveSessionItem({
      schoolKey: auth.schoolKey,
      viewerRole: auth.session.user.role as "admin" | "teacher",
      viewerId: String(auth.session.user.id || "").trim(),
      liveSessionId: id,
    ...
    return NextResponse.json(
      {
        ...
        message:
          error instanceof Error
            ? error.message
            : "Failed to delete the live item.",
      },
      { status: getLiveSessionErrorStatus(error) },
    );
  }
