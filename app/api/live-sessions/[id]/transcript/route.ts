...
import {
  ...
  normalizeLiveSessionTranscriptWriteInput,
...
export async function PATCH(req: NextRequest, { params }: RouteContext) {
  ...
  try {
    ...
    const liveSession = await upsertWorkspaceLiveSessionTranscript({
      schoolKey: auth.schoolKey,
      viewerRole: auth.session.user.role as "admin" | "teacher",
      viewerId: String(auth.session.user.id || "").trim(),
      liveSessionId: id,
      input: normalizeLiveSessionTranscriptWriteInput(body),
    });
