...
function zipPairs(tagTypes: any[], tags: any[]): Pair[] {
  const n = Math.min(
    Array.isArray(tagTypes) ? tagTypes.length : 0,
  ...
  try {
    ...
    for (let i = 0; i < questions.length; i++) {
      ...
      if (!q || typeof q !== "object")
        return bad(`Question ${i + 1}: not an object`);
      if (!q.class || !q.subject || !q.code)
        return bad(
          `Question ${i + 1}: each question must have 'class', 'subject', and 'code'.`,
        );
    ...
    const tagNameList = Array.from(tagNameTypePairs).map(
      (s) => s.split("|||")[0],
    ); // fetch by name only

    const [classDocs, subjectDocs, tagTypeDocs, tagDocs] = await Promise.all([
      ClassModel.find({ name: { $in: classList } }) as Promise<ClassDoc[]>,
      SubjectModel.find({ name: { $in: subjectList } }) as Promise<
        SubjectDoc[]
      >,
      TagTypeModel.find({ name: { $in: tagTypeList } }) as Promise<
        TagTypeDoc[]
      >, // stored lowercased
    ...
    const classMap = new Map<string, ClassDoc>(
      classDocs.map((c: ClassDoc) => [c.name, c]),
    );
    const subjectMap = new Map<string, SubjectDoc>(
      subjectDocs.map((s: SubjectDoc) => [s.name, s]),
    );
    const tagTypeMap = new Map<string, TagTypeDoc>(
      tagTypeDocs.map((tt: TagTypeDoc) => [lc(tt.name), tt]),
    ); // KEYS LOWERCASED
    ...
    if (missingClasses.length) {
      createdClasses = await ClassModel.insertMany(
    ...
    const existingTT = new Set(
      tagTypeDocs.map((tt: TagTypeDoc) => lc(tt.name)),
    );
    ...
    const existingTagKeys = new Set(
      tagDocs.map((t: TagDoc) => `${t.name}|||${String(t.type)}`),
    );
    let createdTags: any[] = [];
    ...
    for (const subjectName of subjectList) {
      ...
      if (!subjectDoc) {
        ...
        const oldIds = new Set(
          (subjectDoc.tags ?? []).map((x: any) => String(x)),
        );
    ...
    for (let i = 0; i < questions.length; i++) {
      ...
      if (!classDoc || !subjectDoc) {
        throw new Error(
          `Class or Subject not found for question: ${toS(q.content).slice(0, 30)}`,
        );
      }
