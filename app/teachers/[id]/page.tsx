...
export default function TeacherDetailPage() {
  ...
  const classNames = (user?.classIds || []).map(
    (classId) => classes.find((c) => c._id === classId)?.name || classId,
  );
  const subjectNames = (user?.subjectIds || []).map(
    (subjectId) => subjects.find((s) => s._id === subjectId)?.name || subjectId,
  );
    ...
    <div className="container py-6 space-y-6">
      <div className="app-page-header-row">
        <div className="app-page-header">
          ...
          <p className="app-page-subtitle">View teacher profile and assigned classes and subjects.</p>
        </div>
      ...
      {loading ? (
        <div className="app-empty-state">Loading teacher details...</div>
      ) : error ? (
