...
export default async function EditQuestionPaperPage({
  ...
  const initialData = buildQuestionPaperInitialData(paper);
  const initialQuestionCount = initialData.sections.reduce(
    (sum: number, section: { questions?: Array<unknown> }) =>
      sum + (Array.isArray(section.questions) ? section.questions.length : 0),
    0,
  );
  const initialTotalMarks = initialData.sections.reduce(
    (
      sum: number,
      section: { questions?: Array<{ marks?: number | null }> },
    ) =>
      sum +
      (Array.isArray(section.questions)
        ? section.questions.reduce(
            (sectionSum, question) => sectionSum + Number(question?.marks ?? 0),
            0,
          )
        : 0),
    0,
