import AdminDetailPageClient from "@/components/workspace/people-detail/AdminDetailPageClient";
import { getWorkspacePeopleUserData } from "@/lib/server/workspace-people";
import { requireWorkspaceStaffSession } from "@/lib/server/workspace-user-directory";

export const dynamic = "force-dynamic";

type AdminDetailPageProps = {
  params: Promise<{ id: string }>;
};

export default async function AdminDetailPage({ params }: AdminDetailPageProps) {
  const { id } = await params;
  const { schoolKey, viewerRole } = await requireWorkspaceStaffSession();

  if (viewerRole !== "admin") {
    return (
      <AdminDetailPageClient
        adminId={id}
        initialUser={null}
        initialClasses={[]}
        initialSections={[]}
        initialSubjects={[]}
        initialLoadError="Forbidden"
      />
    );
  }

  try {
    const detailData = await getWorkspacePeopleUserData({
      schoolKey,
      userId: id,
      includeSubjects: true,
      includeStudentAttempts: false,
    });

    return (
      <AdminDetailPageClient
        adminId={id}
        initialUser={detailData.user}
        initialClasses={detailData.classes}
        initialSections={detailData.sections}
        initialSubjects={detailData.subjects}
      />
    );
  } catch (error) {
    return (
      <AdminDetailPageClient
        adminId={id}
        initialUser={null}
        initialClasses={[]}
        initialSections={[]}
        initialSubjects={[]}
        initialLoadError={
          error instanceof Error ? error.message : "Failed to load admin details."
        }
      />
    );
  }
}
