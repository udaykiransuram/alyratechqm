...
export default function CreateTagPage() {
  ...
  const handleSelectSubject = (subject: Subject) => {
    setSelectedSubjects((prev) => {
      if (!prev.some((item) => item._id === subject._id)) {
        return [...prev, subject];
      }
      return prev;
    });
  ...
  const handleDeselectSubject = (subjectToRemove: Subject) => {
    setSelectedSubjects((prev) => prev.filter((item) => item._id !== subjectToRemove._id));
  };
  ...
  const filteredAvailableSubjects = allSubjects.filter(
    (subject) =>
      !selectedSubjects.some((item) => item._id === subject._id) &&
      ...
      <div className="app-page-shell max-w-6xl px-4 py-5 sm:px-0">
        ...
        <div className="app-editor-grid">
          <div className="app-editor-main">
            <Card className="app-surface overflow-hidden">
              <CardHeader className="app-section-header">
                ...
                <CardDescription>
                  Choose the tag label, its type, and any subjects that should carry it immediately.
                </CardDescription>
              </CardHeader>
                ...
                <div className="space-y-3">
                  ...
                  <Popover open={subjectSearchOpen} onOpenChange={setSubjectSearchOpen}>
                    <PopoverTrigger asChild>
                      <Button
                        variant="outline"
                        className="flex h-10 w-full items-center justify-start px-3 text-left font-normal"
                        ...
                        <CommandList>
                          ...
                          <CommandGroup>
                            {filteredAvailableSubjects.map((subject) => (
                              ...
                              >
                                <Checkbox
                                  checked={selectedSubjects.some((item) => item._id === subject._id)}
                                  className="mr-2"
                                />
          ...
          <aside className="app-editor-aside">
            <Card className="app-surface overflow-hidden">
              <CardHeader className="app-section-header">
                <CardTitle>Tag Strategy</CardTitle>
                <CardDescription>Small decisions here shape filtering quality later.</CardDescription>
              </CardHeader>
              <CardContent className="app-section-body">
                <div className="app-note-list">
                  <div className="app-note-item">
                    Pick short, stable tag names so teachers can reuse them without guessing variants.
                  </div>
                  <div className="app-note-item">
                    Use tag types to separate difficulty, topic, skill, or reporting dimensions.
                  </div>
                  <div className="app-note-item">
                    Assigning subjects now helps keep the subject library and tag library in sync.
                  </div>
                </div>
              </CardContent>
            </Card>
